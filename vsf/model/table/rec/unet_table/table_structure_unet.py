...
class TSRUnet:
    ...
    def postprocess(self, img, pred, **kwargs):
        ...
        ori_shape = img.shape
        pred = np.uint8(pred)
        hpred = copy.deepcopy(pred)  # Implementation detail.
        vpred = copy.deepcopy(pred)  # Implementation detail.
        ...
        colboxes = get_table_line(vpred, axis=1, lineW=col)  # Implementation detail.
        rowboxes = get_table_line(hpred, axis=0, lineW=row)  # Implementation detail.
    ...
    def cal_region_boxes(self, tmp):
        components = _iter_connected_component_coords(tmp < 255)  # Implementation detail.
